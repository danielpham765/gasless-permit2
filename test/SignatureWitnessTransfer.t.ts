...
describe("VaultSignautureWitnessTransfer", function () {
  ...
  describe("SignatureWitnessTransfer", function () {
    it("Should deposit", async function () {
        ...
        // Mint tokens to `owner wallet`. And the `owner` approves allownce for the `PERMIT2_ADDRESS` contract.
        // The `approve()` transaction is executed once for all protocols using Permit2 (the vault contract, kyberswap, uniswap, etc.)
        ...
        // The `owner` prepares a Permit2 signature
        ...
        expect(await vault.depositedBalances(user.address, erc20.address), amount.toString());
